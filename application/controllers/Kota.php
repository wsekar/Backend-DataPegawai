...
class Kota extends AUTH_Controller {
	...
	public function prosesTambah() {
		$this->form_validation->set_rules('kota', 'Kota', 'trim|required|is_unique[kota.nama]', array('is_unique'=>'%s tersebut sudah ada!'));
	...
	public function update() {
		$data['userdata'] 	= $this->userdata;
		$id 				= trim($_POST['id']);
		$data['dataKota'] 	= $this->M_kota->select_by_id(array('id'=>$id));
	...
	public function detail() {
		$data['userdata'] 	= $this->userdata;
		$nmkota 				= trim($_POST['id']);
		$data['kota'] = $this->M_kota->select_by_id(array('nama'=>$nmkota));
		$data['jumlahKota'] = $this->M_kota->total_rows();
		$data['dataKota'] = $this->M_kota->select_by_pegawai($nmkota);
